...
class SubnormasController extends AppController {
    ...
    public function planauditoria($id) {
    $normas = $this->AuditoriasNormageneral->find('all', array('conditions'=>array('AuditoriasNormageneral.auditoria_id'=>$id)));
    foreach ($normas as $norma){
     $normaELegidas[]= $norma['AuditoriasNormageneral']['normageneral_id'];
    }
    //debug($normaELegidas);   
    $subnormas=$this->Subnorma->find('all',array('conditions'=>array('Subnorma.normageneral_id'=>$normaELegidas)));
//    debug($subnormas); 
    foreach ($subnormas as $subnorma){
     $subnormaELegidas= $subnorma['Subnorma']['normageneral_id'];
    }
    $subnormasindices=$this->Subnormaindice->find('list',array('conditions'=>array('Subnormaindice.subnorma_id'=>$subnormaELegidas)));
//    debug($subnormasindices);
    $this->set(compact('subnormasindices','subnormas'));
